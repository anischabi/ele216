...
// déclarés dans l'interface (tabdyn.h).
struct tabdyn {
...
// Accesseurs
int tabdyn_taille(tabdyn_t* tabdyn) { 
    assert(tabdyn != NULL);
    return tabdyn->taille;
}

bool tabdyn_est_vide(tabdyn_t *tabdyn) {
    assert(tabdyn != NULL);
    return tabdyn->taille == 0;
}

int tabdyn_capacite(tabdyn_t* tabdyn) {
    assert(tabdyn != NULL);
